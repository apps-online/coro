            ...
            if (Server::isExistExtension('yaml') !== false) {
                if (self::checkArray($data) !== false) {
                    ...
                    if (! file_exists($output)) {
                        throw new \Coro\Exception\AppException('No exist file Yaml ' . var_dump($output));
                    }
                    $result = true;
                } else {
                    throw new \Coro\Exception\AppException('Valid element array ' . var_export([
                        'output' => $output,
                        'data' => $data
                    ], true));
                }
            } else {
                throw new \Coro\Exception\AppException('No instaled extension YAML');
            }
        } catch (\Coro\Exception\AppException $e) {
            \Coro\AppLogger::LogException($e);
            $result = false;
        }
        return $result;
    }
    ...
    public function arrayToXml($data, $output)
    {
        ...
        try {
            if (Server::isExistExtension('simplexml') !== false) {
                ...
                throw new \Coro\Exception\AppException('Error Server No Installed Extension : SimpleXML');
            }
        } catch (\Coro\Exception\AppException $e) {
            \Coro\AppLogger::LogException($e);
            $result = false;
